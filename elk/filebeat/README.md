...
ماژول دیگر تعبیه شده برای filebeat ماژول iptables می باشد.
~~~
  - module: iptables
    log:
      enabled: true
      var.paths: ["/var/myhost.logs/*"]
      var.input: "file"
~~~
 برای استفاده از این ماژول لازم است که در هنگام تعریف rule ها، برای هر rule مورد نظر
 یک rule دیگر قبل از آن
 (یعنی در ترتیب با اولویت بالاتر)
 قرار دهید تا لاگ های مربوط به آن rule در فایلی مانند `/var/log/kern.log` ذخیره گردد.

 پس از آن از طریق خواندن فایل مربوطه filebeat جمع آوری لاگ را انجام می دهد.
 
 ### add indices
~~~
 https://elk.test.ir/app/management/kibana/indexPatterns
~~~

### [netflow integration](https://gitlab.wanpad.ir/wanpad/elk/-/tree/master/docker-compose/filebeat/netflow)